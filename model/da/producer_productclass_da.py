...
class ProducerProductClassDa(DataBaseManager):
    ...
    def find_by_omniclass_code_and_producer_id(self, omniclass_code, producer_id):
        self.make_engine()
        result = self.session.query(ProducerProductClass).filter(and_(
            ProducerProductClass.omniclass_code == omniclass_code,
            ProducerProductClass.producer_id == producer_id)).all()
        if result:
            return result
